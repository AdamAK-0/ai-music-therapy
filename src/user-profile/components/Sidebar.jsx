...
export default function Sidebar({ children }) {
  ...
  return (
    <div
      className="flex min-h-[calc(100vh-64px)] px-24 gap-6 items-start max-md:flex-col max-md:px-4 max-md:gap-4"
      style={{ backgroundColor: pageBg }}
    >
      <aside
        className="w-[350px] rounded-lg p-6 flex-shrink-0 shadow-sm max-md:w-full"
        ...
        <nav className="px-4 max-md:px-0">
          <ul className="list-none p-0 m-0 flex flex-col max-md:flex-row max-md:flex-wrap max-md:justify-center max-md:gap-2">
            {[
              ...
              return (
                <li key={item.name} className="mb-2 max-md:mb-0">
                  <a
                    href={item.href}
      ...
      <main
        className="flex-grow rounded-lg shadow-sm p-8 max-md:p-6"
